...
describe('Join Game - Unit Tests', () => {
    ...
    test('Successfully joins an open game', () => {
        ...
        const testSaveName = 'test save name 1';
        const playerSlots = [2, 3];
        const response = joinGame(ws, testSaveName, playerSlots);
    ...
    test('throws an error if there are no open game slots', () => {
        ...
        const testSaveName = 'test save name 1';
        const playerSlots = [2, 3];
        expect(() => joinGame(ws, testSaveName, playerSlots)).toThrow(joinGameError);
    })
