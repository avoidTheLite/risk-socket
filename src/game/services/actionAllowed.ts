...
export default function actionAllowed(game: Game, action: WsActions, playerID: number,ws: WebSocket): void {
    ...
    if (game.activePlayerIndex !== playerID) {
        throw new PlayerControlError({ message: `Unable to perform action: ${action}. It is not your turn. Active player is player ${game.activePlayerIndex}: ${game.players[game.activePlayerIndex].name}`});
    }

    const controllingClient: WebSocket = manager.getOwner(game.saveName, game.activePlayerIndex);
    if (controllingClient !== ws) {
        throw new turnError({ message: `Unable to perform action: ${action} for player ${playerID}. You do not control this player`});
    }
